...
void SetGroup(pid, group)
{
    int result = setpgid(pid, group);

    if (result == -1)
    {
        printf("unable to put task into same group as self");
        printf("errno %i\n",(int)errno);
        if (errno == EACCES)
        {
            printf("EACCES: The value of the pid argument matches the process ID of a child process of the calling process and the child process has successfully executed one of the exec functions.\n");
        }
        else if (errno == EINVAL)
        {
            printf("EINVAL: The value of the pgid argument is less than 0, or is not a value supported by the implementation.\n");
        }
        else if (errno == EPERM)
        {
            printf("EPERM: The process indicated by the pid argument is a session leader.\n");
        }
        
        else if (errno == ESRCH)
        {
            printf("ESRCH: The value of the pid argument does not match the process ID of the calling process or of a child process of the calling process..\n");
        }
        else
        {
